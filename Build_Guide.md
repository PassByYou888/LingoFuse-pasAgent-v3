...
> **文档版本**：V5.1（脚本优先 · PATH 前置版）
> **最后更新**：2026-09-24
> **适用平台**：Windows 10/11（64 位）
> **相关文档**（同目录）：
> - 项目总览：`readme.md`
> - Pascal 开发者切入：`Pascal_Integration_Guide.md`
> - 推荐模型：`src/NVIDIA-Nemotron-3-Nano-Omni-30B-A3B-Reasoning-UD-IQ4_XS.md`
...
本文档介绍如何将 LingoFuse-pasAgent-v3 中的 Python 脚本和 Pascal 源代码编译为可执行文件（EXE）。

**核心原则**：

1. **所有编译操作都有现成的脚本**——`src\` 目录下已提供 `build_*.ps1`（Python）和 `build_pascal_agent.bat`（Pascal），**不需要手敲 PyInstaller / lazbuild 命令**。
2. **Pascal 编译必须先把 Lazarus 加入系统 PATH**——`lazbuild.exe` 找不到，脚本就会失败。
3. **代码生成器不在本仓库**——所有 MCP-API 生成工具已独立到专用仓库。
...
> ⚠️ **代码生成器说明**：pasAgent v3 仓库**不再包含**任何代码生成器（MCP-API 生成器）源码。
> 所有代码生成工具已独立到专用仓库，请一律访问：
...
| **Lazarus**（含 Free Pascal 3.2+） | 编译 Pascal 项目 | [Lazarus IDE](https://lazarus-ide.org) |
| **LingoFuse 动态库** | 所有 EXE 的运行时依赖 | 由 LingoFuse 核心项目提供（不在本仓库），或使用预编译包内置的 DLL |

### 1.3 ⚠️ 关键前置：把 Lazarus 加入系统 PATH

> **如果你要用命令行（或 `build_pascal_agent.bat` 脚本）编译 Pascal 项目，`lazbuild.exe` 必须在系统 PATH 中。否则脚本会报“找不到 lazbuild.exe”。**

**如何确认 `lazbuild.exe` 是否已在 PATH 中**：
...
- ✅ 输出版本号（例如 `2.2.6`）→ 已在 PATH 中，**可以跳过本节**。
- ❌ 报 `The term 'lazbuild' is not recognized...` → 需要按下方步骤加入 PATH。

**如何把 Lazarus 加入系统 PATH**：

1. 找到 Lazarus 的安装目录（默认通常是 `C:\Lazarus`，也可能是 `D:\Lazarus` 或其他自定义位置）。
2. 确认 `lazbuild.exe` 的完整路径。典型位置：
   - `C:\Lazarus\lazbuild.exe`
   - `D:\Lazarus\lazbuild.exe`
3. **方式 A：永久加入系统 PATH（推荐）**
   - 按 `Win + R` → 输入 `sysdm.cpl` → 回车
   - 「高级」标签页 → 「环境变量」按钮
   - 在「系统变量」或「用户变量」中找到 `Path` → 双击 → 「新建」→ 粘贴 Lazarus 的安装目录（例如 `C:\Lazarus`）→ 确定
   - **关闭所有已打开的命令行窗口**，重新开一个 PowerShell，再执行 `lazbuild --version` 验证
4. **方式 B：临时加入当前会话（仅本窗口有效）**

   ```powershell
   $env:PATH = "C:\Lazarus;$env:PATH"
   lazbuild --version
   ```

   > 只对当前 PowerShell 窗口有效，关闭后失效。**每次新开窗口都要重新设置**，不推荐用于日常开发。

**如果不想用命令行编译**：也可以直接用 Lazarus IDE 打开 `.lpi` 文件，按 `Ctrl+F9` 编译——这种方式**不依赖 PATH**。

### 1.4 图 1：编译工具链全景

```mermaid
flowchart LR
    A["📄 Python 源码<br/>.py"] --> B["⚙️ build_*.ps1"]
    B --> C["🎯 Python EXE"]

    D["📄 Pascal 源码<br/>.lpr + .lpi"] --> E["⚙️ build_pascal_agent.bat<br/>（需 lazbuild 在 PATH）"]
    E --> F["🎯 Pascal EXE"]
...
└── src\                             ← 所有源码与编译脚本
    ...
    ├── llm_client_v3_Structured_Output_Learning_Guide.md  # Structured Output 学习指南
    ...
    ├── lf_http_bridge_client.pas    # HTTP 桥接客户端单元
    ├── pascal_agent_service.lpr     # 信标（主程序）
    ├── pascal_agent_service.lpi     # 信标（Lazarus 项目）
    ├── pascal_agent_service_unit.pas # 信标单元
    ├── pascal_agent_api.lpr         # 工具提供者示例（主程序）
    ├── pascal_agent_api.lpi         # 工具提供者示例（Lazarus 项目）
    ...
    ├── NVIDIA-Nemotron-3-Nano-Omni-30B-A3B-Reasoning-UD-IQ4_XS.gguf  # 推荐模型（可选）
    ├── NVIDIA-Nemotron-3-Nano-Omni-30B-A3B-Reasoning-UD-IQ4_XS.md    # 推荐模型说明
    ├── QUICK_START_LLM_STACK.md     # LLM 栈快速上手
    ...
    │   ├── json_repair_preprocess.py / lf_io.py / network_events.py
    │   ├── test_lingofuse.py / test_bridge.py
    │   ├── Bridge_User_Guide.md
    │   └── json_repair\             # JSON 修复子系统
    │       ├── json_parser.py / json_repair.py / parser_schema.py
    │       ├── parse_array.py / parse_object.py / parse_string.py
    │       ├── schema_repair.py / parser_parenthesized.py / parse_comment.py
    │       ├── parse_number.py
...
- **所有编译动作都有现成脚本**——Python 侧是 `build_*.ps1`，Pascal 侧是 `build_pascal_agent.bat`。**不需要手敲 PyInstaller / lazbuild 命令**。
- **`llm_common\` 和 `lingofuse\` 是本仓库自带的 Python 包**，被 `llm_service.py` / `llm_proxy.py` / `llm_proxy_tool.py` 共享。打包脚本已自动收集。
- **本仓库不再包含任何代码生成器源码**——所有 MCP-API 生成工具已独立到 [LingoFuse-Tools](https://github.com/PassByYou888/LingoFuse-Tools) 仓库。
...
如果你已经安装好所有前置软件，**并且已经按 1.3 节把 Lazarus 加入 PATH**，执行以下步骤即可：

### 图 2：快速编译流程

```mermaid
flowchart TB
    S0["0️⃣ 把 Lazarus 加入 PATH<br/>（1.3 节）"] --> S1["1️⃣ 安装 Python 依赖"]
    S1 --> S2["2️⃣ 运行 build_*.ps1<br/>编译 Python 组件"]
    S2 --> S3["3️⃣ 运行 build_pascal_agent.bat<br/>编译 Pascal 组件"]
    S3 --> S4["4️⃣ 部署动态库"]
    S4 --> S5["✅ 完成"]

    style S0 fill:#922B21,stroke:#5A1A14,stroke-width:4px,color:#FFFFFF
...
# 0. 确认 lazbuild 在 PATH 中（Pascal 编译前置）
lazbuild --version
# 如果报"not recognized"，先按 1.3 节把 Lazarus 加入 PATH，再回来

# 1. 安装 Python 依赖
pip install -r requirements.txt
pip install pyinstaller

# 2. 编译 Python 组件（按需选择，脚本会自动调用 PyInstaller）
...
# 3. 编译 Pascal 组件（脚本内部逐个调用 lazbuild）
.\build_pascal_agent.bat
#   → pascal_agent_service.exe
#   → pascal_agent_api.exe
#   → CreateHealthCheck\HealthCheck.exe
```

编译产物在 `src\dist\` 目录下。

> 💡 **提示**：**不需要手敲 `pyinstaller` 或 `lazbuild` 命令**——所有参数（依赖收集、hidden-import、资源文件等）都已在脚本里配置好。

---

## 三、Python 组件编译

所有 Python 脚本均通过 **`src\` 目录下现成的 `build_*.ps1` 脚本**打包为独立 EXE。脚本内部已配置好必要的 PyInstaller 参数（依赖收集、数据文件、hidden-import 等），**你只需要运行脚本**。

### 3.1 环境准备

打开 PowerShell，进入 `src` 目录：
...
- 每个目标都使用 `--onefile --noconfirm --clean --paths .`
- 每个目标都收集 `lingofuse` 和 `llm_common` 子模块
- `llm_proxy_tool.py` 额外声明 `--hidden-import language_middleware`（源码里是 `try/except` 导入）
...
- 使用 `src\lingofuse\bridge.py` 作为源文件（注意不是 `src\` 根目录）
- 收集 `lingofuse` 子模块、`flask`、`jinja2`
- 保留 `--hidden-import requests`：`bridge.py` 的 `--debug` 分支可能探测后端 `/v1/models`

---

## 四、Pascal 组件编译

Pascal 项目通过 **`src\build_pascal_agent.bat`** 一键编译。

> ⚠️ **前置条件**：`lazbuild.exe` 必须在系统 PATH 中。
>
> **如果还没配 PATH**：
> - 要么按 1.3 节把 Lazarus 加入 PATH；
> - 要么直接用 Lazarus IDE 打开 `.lpi` 文件，按 `Ctrl+F9` 编译（此方式不依赖 PATH）。

### 4.1 使用 build_pascal_agent.bat 一键编译
...
### 4.2 单独编译某个 Pascal 项目

如果只想编译某一个项目，**有两条路径**：

**路径 A：命令行（需要 `lazbuild` 在 PATH 中）**

```cmd
lazbuild.exe -B .\pascal_agent_service.lpi
```

**路径 B：使用 Lazarus IDE（不依赖 PATH）**

1. 打开 Lazarus IDE
2. `文件` → `打开` → 选择 `.lpi` 文件
3. 按 `Ctrl+F9`（或菜单 `运行` → `构建`）

### 4.3 编译 `llm_client_v3`（Pascal SDK）
...
`llm_tool_v3.lpi` 是一个完整的 Lazarus 项目。

- **命令行（需要 `lazbuild` 在 PATH 中）**：
...
- **Lazarus IDE**：打开 `llm_tool_v3.lpi`，按 `Ctrl+F9`。
...
flowchart LR
    ...
    B --> C{"lazbuild 在 PATH？"}
    C -->|"是"| D["⚙️ build_pascal_agent.bat<br/>（或手动 lazbuild）"]
    C -->|"否"| E["🖥️ Lazarus IDE<br/>打开 .lpi → Ctrl+F9"]
    D --> F["🎯 编译产物 EXE / 单元"]
    E --> F
    ...
    style C fill:#B7791F,stroke:#7E5109,stroke-width:4px,color:#FFFFFF
...
### Q2：执行 `build_pascal_agent.bat` 时提示 `'lazbuild.exe' 不是内部或外部命令`

**原因**：Lazarus 没有加入系统 PATH。

**解决**：

- 按 1.3 节把 Lazarus 安装目录加入系统 PATH，然后**重新打开一个 PowerShell 窗口**。
- 或者：直接用 Lazarus IDE 打开 `.lpi` 文件，按 `Ctrl+F9` 编译。
...
### Q3：PyInstaller 打包后运行报 `ModuleNotFoundError`
...
- `lingofuse` 包（含 `json_repair\` 子模块）用 `--collect-submodules lingofuse` 收集。

### Q4：Pascal 编译报 `Can't find unit Z.Core`
...
- 确保使用 `lazbuild`（或 Lazarus IDE）编译，因为 `.lpi` 中已配置单元搜索路径。
...
- Python 侧：可在 `build_*.ps1` 中使用 `--upx-dir` 参数启用 UPX 压缩（需先下载 UPX）。但 `--onefile` 模式本身会增大启动解压开销，若体积敏感，可改用 `--onedir` 模式。
- Pascal 侧：在 Lazarus 项目选项中开启「编译时优化」或使用 UPX 压缩。

### Q6：使用 `--generate-configs` 时找不到 `generate_agent_json` 模块

**解决**：

- 确保 `generate_agent_json.py` 在 `src\` 目录（与 `mcp_api_tool.py` 同目录）。
- 打包脚本 `build_mcp_api_tool.ps1` 已包含 `--hidden-import generate_agent_json`。

### Q7：`build_llm_service.ps1` 编译时找不到 `llama_cpp` 模块
...
### Q9：如何编译 `llm_client_v3` / `llm_tool_v3`？

**A**：

- `llm_client_v3.pas` 是 **SDK 单元**，不需要单独编译——把它加入你的 Pascal 项目搜索路径，在 `uses` 段引用即可（详见 4.3 节）。
- `llm_tool_v3.lpi` 是一个完整的 Lazarus GUI 项目。
  - **有 PATH**：`lazbuild.exe -B .\llm_tool_v3.lpi`
  - **无 PATH**：用 Lazarus IDE 打开 `llm_tool_v3.lpi`，按 `Ctrl+F9`
- 编译前确保 git 子模块 `zCore\` 已初始化。

### Q10：代码生成器在哪里？如何编译？

**A**：pasAgent v3 仓库**不再包含**任何代码生成器（MCP-API 生成器）源码。

所有代码生成工具已独立到专用仓库，请一律访问：

### 👉 [https://github.com/PassByYou888/LingoFuse-Tools](https://github.com/PassByYou888/LingoFuse-Tools)

该仓库包含生成器源码、声明规范、使用手册与预编译包。
...
> - `llm_tool_v3.exe` 是 **Pascal GUI 程序**，与 Python 打包脚本无关，需通过 `lazbuild` 或 Lazarus IDE 单独编译。
> - **MCP-API 生成器**（如 `code_decl_to_mcp.exe`）**不在本仓库**，请从 [LingoFuse-Tools](https://github.com/PassByYou888/LingoFuse-Tools) 仓库获取。
...
| `src/LingoFuse_Pascal_Complete_Guide.md` | Pascal 核心层完整指南（含踩坑知识库） |
| `src/llm_client_v3.md` | Pascal 客户端 SDK 文档 |
| `src/llm_client_v3_Structured_Output_Learning_Guide.md` | Structured Output 完整学习指南 |
| `src/QUICK_START_LLM_STACK.md` | LLM 栈快速上手 |
| `src/NVIDIA-Nemotron-3-Nano-Omni-30B-A3B-Reasoning-UD-IQ4_XS.md` | 推荐模型下载与部署 |
...
**文档版本**：V5.1（脚本优先 · PATH 前置版——全文改为“运行脚本，不手敲命令”；新增 1.3 节强调 Lazarus 加入 PATH 的前置条件；Pascal 编译章节新增“有 PATH / 无 PATH”两条路径；移除代码生成器源码章节）
